...
?>  
    ...
    <?php 
        ...
        if(isset($_GET['category_id']))
        {
            ...
            if($category_id == $sn)
            {
                $sql_category_name = "SELECT categories.name, category_image FROM categories WHERE category_id = '$sn'";

                $res_category_name = mysqli_query($conn, $sql_category_name);
                            
                $count_category_name = mysqli_num_rows($res_category_name);

                if($count_category_name == 1)
                {
                $category_name_row = mysqli_fetch_assoc($res_category_name);
                $category_name1 = $category_name_row['name'];
                $category_image = $category_name_row['category_image'];
                }
            }
        }
    ?>
    ...
    <section class="popular-choices padding-brands padding-content">
        <div class="container padding-content">
            <div class="text-center padding-title">
                <h1>Browse <?php echo $category_name1 ?></h1>
            </div>
            <div class="row">
            ...
            <?php
                ...
                if($count_bikes>0)
                {
                    while($row_bikes=mysqli_fetch_assoc($res_bikes))
                    {
                        ...
                        {
                            echo "<div class='notice-text-failed text-center'>No Bikes Added to $category_name1</div>";
                        }
                    ?>
